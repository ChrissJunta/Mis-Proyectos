...
switch ($op) { 
    case 'select':
        ...
        $condicion=$condicion."where propi_metros like '%".$filtro."%' OR propi_ciudad like '%".$filtro."%' ";
            ...
            $resultqry = mysqli_query($con,"SELECT menu.comida, alimentos.alimento, alimentos.porcion FROM menu,alimentos,dieta where menu.idm=alimentos.idm and dieta.idd=menu.idd and dieta.idd='d1'".$condicion );
            if (!$resultqry) {
            ...
            case 'select1':
                ...
                $condicion=$condicion."where propi_metros like '%".$filtro."%' OR propi_ciudad like '%".$filtro."%' ";
                    ...
                    $resultqry = mysqli_query($con,"SELECT paciente.nombresp, dieta.nombred, resultado FROM paciente,dieta,die_paci where dieta.idd=die_paci.idd and paciente.idp=die_paci.idp and die_paci.idp='p1'".$condicion );
                    if (!$resultqry) {
                    echo json_encode("Ocurrió un error en la consulta");
                    exit;
                    }
