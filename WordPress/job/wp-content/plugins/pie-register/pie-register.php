		...
		public function pie_enable_registration_msg()
		{
			...
			?>
				<div class="notice notice-info">
					<div>
						<p>
							<?php echo __( 'User registration currently disabled. To enable, Go to ','pie-register'); ?>
							<a href="<?php echo esc_url($general_settings_page); ?>" >
								<?php  _e( "Settings -> General", 'pie-register' ) ?>
							</a>
							<?php echo __( 'and under Membership, check "Anyone can register".','pie-register'); ?>
						</p> 
					</div>
				</div>
			<?php
...
?>
		...
		function pie_save_registration()
		{
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				if ( empty($pass) )
				{
					update_user_meta( $user_id, 'email_user_password', true);
				}				
				/*
					*	Check Pricing
				*/
				...
				*/
				...
				else if($confirmation_type == "" || $confirmation_type == "text" )
				{
					if(empty($this->pie_post_array['registration_success']))
					{
						...
						{
							$this->pie_post_array['registration_success']	= __("Thank you for registering. Password has been emailed to you.","pie-register");
						}else{
		...
		function InsertCode($name)
		{
				...
				$desc  = trim($_POST['invitation_code_description']);
				...
				if(!$wpdb->query( $wpdb->prepare("INSERT INTO ".$codetable." (`created`,`modified`,`name`,`count`,`status`,`code_usage`,`expiry_date`,`code_description`)VALUES(%s,%s,%s,%s,%s,%s,%s,%s)", $date, $date, $name, $counts, "1", $usage, $expiry_date,$desc) )){
					$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}
				$wpdb->flush();
				return true;
