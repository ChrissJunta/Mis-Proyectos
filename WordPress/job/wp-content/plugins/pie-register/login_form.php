			...
			elseif ( 'activate' == $action )
			{
				...
				if(sizeof($unverified )==1)
				{
					...
					if($user_login == $_GET['pie_id'])
					{
						...
						{
							$login_success = '<strong>'.ucwords(__("success","pie-register")).'</strong>: '.apply_filters("piereg_your_account_is_now_active",__("Your account is now active. Please check your email for login credentials.","pie-register"));
